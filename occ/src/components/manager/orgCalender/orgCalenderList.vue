<template>
  <div class="publicList">
    <div class="publicTitle">机构工作日历</div>
    <div class="publicListTable">
      <div class="publicList_Head">
        <div class="publicList_Head_Bt">
          <el-button @click="jumpTo('/manager/orgCalenderForm/add')" style="background-color: lightblue" v-if="hasPermission('orgCalender_add')">新增</el-button>
            <el-button @click="displayInfo" style="background-color: #ccc">刷新</el-button>
          <el-button @click="deleteAll" style="background-color: #ff000096" v-if="hasPermission('orgCalender_delete')">批量删除</el-button>
        </div>
      ...
      <el-table :data="datas.list" ref="table">
        ...
        <el-table-column
          ...
          width="150">
          <template slot-scope="scope">
            <el-button @click="dis(scope.row.id)" type="text" size="mid" v-if="hasPermission('orgCalender_display')">查看</el-button>
            <el-button type="text" size="mid" @click="edit(scope.row.id)" v-if="hasPermission('orgCalender_edit')">编辑</el-button>
            <el-button @click="del(scope.row.id)" type="text" size="mid" v-if="hasPermission('orgCalender_delete')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default{
    ...
    methods: {
      ...
      displayInfo: function(){
        ...
        this.$axios.get('/occ/orgCalender/listByKey',{
      ...
      del: function(orgCalenderId){
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          this.$axios.get('/occ/orgCalender/delete',{
      ...
      deleteAll: function(){
        ...
        this.$confirm('此操作将永久删除所选数据, 是否继续?', '提示', {
          ...
          this.$store.commit('showLoading');
          this.$axios.post('/occ/orgCalender/deleteBatch',arr).then(res =>{
            this.$store.commit('hideLoading');
            this.$message({
              type: 'success',
              message: '删除成功!'
      ...
      edit: function(id){
        this.$router.push('/manager/orgCalenderForm/edit/' + id);
      },
      dis: function(id){
        this.$router.push('/manager/orgCalenderDisplay/' + id);
      },
