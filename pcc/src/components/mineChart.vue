<template>
  <div class="full flex-center flex-row" v-bind:onresize="resizeCharts">
    <div id="week" class="full"></div>
    <div id="week11" class="full"></div>
    <div id="week12" class="full"></div>
    <div id="week13" class="full"></div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$axios.get('/pcc/schedule/day/count', {
          ...
          .then(res => {
            ...
            if (data.data == null) {
              ...
              echartsContainer.weekEchart(this.parseWeekData(data.data, date), this.charts, 'week')
              echartsContainer.weekEchart(this.parseWeekData(data.data, date), this.charts, 'week11')
              echartsContainer.weekEchart(this.parseWeekData(data.data, date), this.charts, 'week12')
              echartsContainer.weekEchart(this.parseWeekData(data.data, date), this.charts, 'week13')
            }
      ...
      parseWeekData: function(data, date) {
        let result = {
          data: [],
          maxCount: 0,
          counts:[],
